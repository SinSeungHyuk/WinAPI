...
CAssetMgr::~CAssetMgr()
{
	Release_UnorderedMap(m_mapTex);
	Release_UnorderedMap(m_mapSound);
}
...
CTexture* CAssetMgr::FindTexture(const wstring& _strKey)
{
	...
	unordered_map<wstring, CTexture*>::iterator iter = m_mapTex.find(_strKey);
...
CSound* CAssetMgr::FindSound(const wstring& _strKey)
{
	...
	unordered_map<wstring, CSound*>::iterator iter = m_mapSound.find(_strKey);
