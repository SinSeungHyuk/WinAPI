...
CAnimator::CAnimator(const CAnimator& _Other)
	...
	, isFinish(false)
{
	unordered_map<wstring, CAnim*>::const_iterator iter = _Other.m_mapAnim.begin();
...
CAnimator::~CAnimator()
{
	Release_UnorderedMap(m_mapAnim);
}
...
{
	unordered_map<wstring, CAnim*>::iterator iter = m_mapAnim.find(_Name);
...
void CAnimator::SaveAnimation(const wstring& _RelativeFolder)
{
	...
	unordered_map<wstring, CAnim*>::iterator iter = m_mapAnim.begin();
